...
{
	PixelNumber width = (DISPLAY_X - 2 * margin + fieldSpacing)/numCols - fieldSpacing;
...
void CreatePrintingTabFields(const ColourScheme& colours)
{
	...
	DisplayField::SetDefaultColours(colours.buttonTextColour, colours.buttonTextBackColour);
	babystepButton = new TextButton(row7, babystepColumn, DISPLAY_X - babystepColumn - fieldSpacing, strings->babystep, evBabyStepPopup);
	...
	resetButton = new TextButton(row7, cancelColumn, DISPLAY_X - cancelColumn - margin, strings->cancel, evReset, "M0");
	mgr.AddField(resetButton);

	//		DisplayField::SetDefaultColours(labelTextColour, defaultBackColour);
	//		mgr.AddField(printingField = new TextField(row8, margin, DISPLAY_X, TextAlignment::Left, "printing ", printingFile.c_str()));

	DisplayField::SetDefaultColours(colours.progressBarColour,colours. progressBarBackColour);
	mgr.AddField(printProgressBar = new ProgressBar(row8 + (rowHeight - progressBarHeight)/2, margin, progressBarHeight, DISPLAY_X - 2 * margin));
...
void CreateMessageTabFields(const ColourScheme& colours)
{
	...
	DisplayField::SetDefaultColours(colours.buttonTextColour, colours.buttonImageBackColour);
	mgr.AddField(new IconButton(margin,  DISPLAY_X - margin - keyboardButtonWidth, keyboardButtonWidth, IconKeyboard, evKeyboard));
	DisplayField::SetDefaultColours(colours.labelTextColour, colours.defaultBackColour);
	mgr.AddField(new StaticTextField(margin + labelRowAdjust, margin, DISPLAY_X - 2 * margin - keyboardButtonWidth, TextAlignment::Centre, strings->messages));
...
void CreateSetupTabFields(uint32_t language, const ColourScheme& colours)
{
	...
	mgr.AddField(fwVersionField = new TextField(row1, margin, DISPLAY_X, TextAlignment::Left, strings->firmwareVersion, VERSION_TEXT));
	mgr.AddField(freeMem = new IntegerField(row2, margin, DISPLAY_X/2 - margin, TextAlignment::Left, "Free RAM: "));
	mgr.AddField(touchX = new IntegerField(row2, DISPLAY_X/2, DISPLAY_X/4, TextAlignment::Left, "Touch: ", ","));
	mgr.AddField(touchY = new IntegerField(row2, (DISPLAY_X * 3)/4, DISPLAY_X/4, TextAlignment::Left));

	DisplayField::SetDefaultColours(colours.errorTextColour, colours.errorBackColour);
	mgr.AddField(settingsNotSavedField = new StaticTextField(row3, margin, DISPLAY_X - 2 * margin, TextAlignment::Left, strings->settingsNotSavedText));
...
void CreateMainPages(uint32_t language, const ColourScheme& colours)
{
	...
	mgr.AddField(nameField = new StaticTextField(row1, 0, DISPLAY_X - statusFieldWidth, TextAlignment::Centre, machineName.c_str()));
	mgr.AddField(statusField = new StaticTextField(row1, DISPLAY_X - statusFieldWidth, statusFieldWidth, TextAlignment::Right, nullptr));
